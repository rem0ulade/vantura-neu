...
const proof = [
  { title: 'Enterprise experience', text: 'Internal software, reporting, calculation and decision-support tools built around real business processes in German and international enterprise environments.' },
...
const projects = [
  ...
  },
  {
    ...
    description: 'Business-critical reporting, forecasting, calculation and decision-support tools developed across German and international organisations. Client data, interfaces and implementation details remain confidential; responsibilities and technical scope can still be discussed.',
      ...
      <header className="fixed inset-x-0 top-0 z-50 border-b border-slate-200/70 bg-[#f7f9fc]/80 backdrop-blur-xl">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 lg:px-8">
          ...
          <nav className="flex items-center gap-5 text-xs font-semibold uppercase tracking-[.14em] text-slate-500">
            ...
            <div className="inline-flex items-center rounded-full border border-slate-200 bg-white p-1 text-[10px] font-semibold tracking-normal"><Link href="/Portfolio/" className="rounded-full bg-slate-950 px-2.5 py-1.5 text-white">EN</Link><Link href="/de/Portfolio/" className="rounded-full px-2.5 py-1.5 text-slate-500 transition hover:text-slate-950">DE</Link></div><a href="https://github.com/rem0ulade" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 transition hover:text-slate-950"><Github size={15}/> GitHub</a>
          </nav>
        </div>
      </header>
              ...
              {projects.map((project) => {
                ...
                const card = <div data-tilt className={`showcase-tilt relative overflow-hidden rounded-[34px] border border-black/5 ${project.tone} p-7 sm:p-10 lg:p-12`}>
                  <div className="relative z-10 grid min-h-[430px] gap-10 lg:grid-cols-[.8fr_1.2fr]">
                    <div className="min-w-0 flex flex-col justify-between"><div className="flex items-center justify-between"><span className="text-xs font-semibold uppercase tracking-[.18em] opacity-55">{project.index} / {project.label}</span><Icon size={25} className="opacity-45"/></div><div><p className="text-sm font-medium opacity-55">{project.subtitle}</p><h3 className="mt-3 max-w-full hyphens-auto [overflow-wrap:anywhere] text-[clamp(2.6rem,12vw,4.5rem)] font-semibold leading-[.94] tracking-[-.055em] sm:text-7xl">{project.title}</h3></div></div>
                    <div className="min-w-0 flex flex-col justify-end"><p className="max-w-2xl hyphens-auto [overflow-wrap:anywhere] text-xl leading-8 opacity-75 sm:text-2xl sm:leading-9">{project.description}</p><div className="mt-8 flex flex-wrap gap-2">{project.tags.map((tag)=><span key={tag} className="rounded-full border border-current/15 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[.1em] opacity-60">{tag}</span>)}</div>{project.logos && <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">{project.logos.map((logo)=><div key={logo.src} className="flex h-20 items-center justify-center rounded-2xl border border-current/10 bg-white/75 p-4"><img src={logo.src} alt={logo.alt} className="h-full w-full object-contain" /></div>)}</div>}{project.href && <div className="mt-9 inline-flex items-center gap-2 text-sm font-semibold">Open product <ArrowUpRight size={16}/></div>}</div>
