...
const copy = {
  en: {
    ...
    items: [
      ...
      { word: 'AI', title: 'AI Projects & AI Officer', label: 'AI Lab', description: 'Dashboards, internal tools, automations and AI workflows — plus strategy, governance and ongoing AI support when a project grows beyond implementation.', href: '/ai/', icon: BrainCircuit, visual: 'ai' },
  ...
  de: {
    ...
    items: [
      ...
      { word: 'AI', title: 'KI-Projekte & AI Officer', label: 'AI Lab', description: 'Dashboards, interne Tools, Automationen und KI-Workflows — plus Strategie, Governance und laufende Begleitung, wenn aus einem Projekt mehr wird.', href: '/de/ai/', icon: BrainCircuit, visual: 'ai' },
...
function Preview({ type }: { type: string }) {
  ...
  if (type === 'ai') return <div className="flex h-full items-center justify-center p-8"><div className="w-full max-w-md rounded-[30px] bg-slate-950 p-6 text-white shadow-2xl"><div className="flex items-center justify-between"><div><div className="text-[10px] font-semibold uppercase tracking-[.18em] text-white/35">AI projects & guidance</div><div className="mt-1 text-lg font-semibold">Build · Improve · Scale</div></div><BrainCircuit className="text-blue-300" size={22}/></div><div className="mt-6 space-y-3">{[['Dashboards','Decision tools'],['Automation','AI workflows'],['Scale','Strategy & governance']].map(([from,to],index)=><div key={from} className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 rounded-2xl border border-white/10 bg-white/[.05] px-4 py-3"><span className="text-xs text-white/55">{from}</span><ArrowRight size={12} className="text-white/20"/><span className={`rounded-lg px-2 py-1.5 text-right text-[10px] font-semibold ${index===0?'bg-emerald-300/10 text-emerald-200':'bg-blue-300/10 text-blue-200'}`}>{to}</span></div>)}</div></div></div>
  return null
}

export function LocalizedLanding({ locale, test = false }: { locale: 'en' | 'de'; test?: boolean }) {
  const t = copy[locale]
  ...
  return <div className="min-h-screen overflow-hidden bg-[#f7f9fc] text-ink">
    {!test && <LanguageRedirect locale={locale}/>}<div className="pointer-events-none fixed inset-0 bg-[radial-gradient(circle_at_15%_10%,rgba(59,130,246,.12),transparent_30%),radial-gradient(circle_at_85%_20%,rgba(124,58,237,.09),transparent_28%),radial-gradient(circle_at_50%_100%,rgba(14,165,233,.08),transparent_34%)]"/>
    <header className="relative z-30"><div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 lg:px-8"><Link href={test?'/test/':locale==='de'?'/de/':'/'} className="text-base font-semibold tracking-[-.03em]">Vantura Studios</Link><div className="flex items-center gap-5"><Link href={locale === 'de' ? '/de/Portfolio/' : '/portfolio/'} className="hidden text-xs font-semibold uppercase tracking-[.14em] text-slate-400 transition hover:text-slate-950 sm:block">Portfolio <span aria-hidden="true">↗</span></Link>{!test && <LanguageSwitch locale={locale}/>}</div></div></header>
    <main className="relative z-10 mx-auto max-w-7xl px-5 pb-16 pt-6 lg:px-8 lg:pb-20 lg:pt-10">
      ...
      <section className="mt-12 hidden min-h-[560px] grid-cols-[.8fr_1.2fr] gap-12 lg:grid"><nav className="flex flex-col justify-center border-y border-slate-200/80 py-5" aria-label="Studio spaces">{t.items.map((item,index)=><Link key={item.word} href={item.href} onMouseEnter={()=>setActive(index)} onFocus={()=>setActive(index)} className={`group flex items-center justify-between border-b border-slate-200/70 py-5 text-left transition first:border-t ${active===index?'text-slate-950':'text-slate-400 hover:text-slate-700'}`}><span className={`text-5xl font-semibold tracking-[-.055em] transition duration-500 xl:text-6xl ${active===index?'translate-x-3':'group-hover:translate-x-2'}`}>{item.word}</span><span className={`text-xs font-semibold uppercase tracking-[.18em] transition ${active===index?'opacity-100':'opacity-0 group-hover:opacity-60'}`}>0{index+1}</span></Link>)}</nav><Link href={selected.href} className="group relative isolate overflow-hidden rounded-[38px] border border-white/80 bg-white/75 shadow-[0_30px_100px_rgba(15,23,42,.12)] backdrop-blur-xl"><div className="absolute inset-0 -z-10 bg-gradient-to-br from-white via-white/75 to-blue-50/60"/><div className="grid h-full grid-rows-[1fr_auto]"><div className="relative min-h-[360px] overflow-hidden"><Preview type={selected.visual}/></div><div className="border-t border-white/80 bg-white/75 p-8 backdrop-blur-xl"><div className="flex items-start justify-between gap-6"><div><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl border border-white bg-white text-blue-600 shadow-sm"><SelectedIcon size={20}/></span><p className="text-xs font-semibold uppercase tracking-[.18em] text-muted">{selected.label}</p></div><h2 className="mt-5 text-4xl font-semibold tracking-[-.045em]">{selected.title}</h2><p className="mt-3 max-w-xl text-base leading-7 text-muted">{selected.description}</p></div><ArrowUpRight className="mt-2 text-slate-400 transition duration-300 group-hover:translate-x-1 group-hover:-translate-y-1 group-hover:text-slate-950"/></div><p className="mt-7 text-sm font-semibold">{t.open} →</p></div></div></Link></section>
      <section className="mt-10 space-y-4 lg:hidden" aria-label="Studio spaces">{t.items.map((item,index)=>{const Icon=item.icon;return <Link key={item.word} href={item.href} className="group block overflow-hidden rounded-[28px] border border-white/80 bg-white/80 shadow-[0_16px_48px_rgba(15,23,42,.08)] backdrop-blur-xl"><div className="p-5"><div className="flex items-start justify-between"><span className="grid h-10 w-10 place-items-center rounded-xl border border-white bg-white text-blue-600 shadow-sm"><Icon size={19}/></span><span className="text-xs font-semibold text-slate-300">0{index+1}</span></div><p className="mt-6 text-[11px] font-semibold uppercase tracking-[.18em] text-muted">{item.label}</p><h2 className="mt-1 text-[34px] font-semibold tracking-[-.05em]">{item.word}</h2><p className="mt-2 text-sm leading-6 text-muted">{item.description}</p><div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4"><span className="text-sm font-semibold">{item.title}</span><ArrowUpRight size={18} className="text-slate-400"/></div></div></Link>})}</section>
      <section className="mt-12 border-t border-slate-200/80 pt-7"><Link href={locale === 'de' ? '/de/Portfolio/' : '/portfolio/'} className="group flex flex-col gap-5 rounded-[24px] px-1 py-3 transition sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-slate-400">Portfolio</p><h2 className="mt-2 text-xl font-semibold tracking-[-.03em] text-slate-800">{t.portfolio}</h2><p className="mt-1 text-sm leading-6 text-slate-500">{t.portfolioText}</p></div><span className="inline-flex shrink-0 items-center gap-2 text-sm font-semibold text-slate-500 transition group-hover:text-blue-600">{t.portfolioCta}<ArrowRight size={16} className="transition group-hover:translate-x-1"/></span></Link></section>
    </main>
  </div>
}
